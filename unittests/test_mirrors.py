...
def test_mirror_init(clean_root, mount_path, systems_path, mirror_ok):
    ...
    # the writable concretizer cache (persists concretization results)
    assert mirrors_obj.concretizer_cache == {"path": "/scratch/spack-concretizer", "description": ""}
...
def test_local_caches_config(tmp_path, clean_root, mount_path, mirror_ok):
    ...
    assert config_data == {"config": {"source_cache": "/scratch/spack-sources"}}

    concretizer_data = yaml.safe_load(files[tmp_path / "concretizer.yaml"])
    assert concretizer_data == {
        "concretizer": {"concretization_cache": {"enable": True, "url": "/scratch/spack-concretizer"}}
    }


def test_concretizer_cache_only(tmp_path, clean_root, mount_path):
    """A mirror file with a concretizer cache but no source cache emits only concretizer.yaml."""
    ...
    concretizer_data = yaml.safe_load(files[tmp_path / "concretizer.yaml"])
    assert concretizer_data == {
        "concretizer": {"concretization_cache": {"enable": True, "url": "/scratch/only-concretizer"}}
    }


def test_concretizer_cache_skipped_on_spack_1_0(tmp_path, clean_root, mount_path):
    """The concretizer cache is not emitted for spack 1.0 (which rejects the config key)."""
    ...
    assert mirrors_obj.concretizer_cache is not None
    assert tmp_path / "concretizer.yaml" not in files


def test_local_caches_absent(tmp_path, clean_root, mount_path, systems_path):
    """No config.yaml or concretizer.yaml is generated when neither cache is configured."""
