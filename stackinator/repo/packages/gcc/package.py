...
class Gcc(AutotoolsPackage, GNUMirrorPackage):
    ...
    patch(
        "patch-745dae5923aba02982563481d75a21595df22ff8.patch",
    ...
    patch(
        "0001-tree-optimization-110221-SLP-and-loop-mask-len.patch",
        sha256="189df11e94ff1e467322eb4f317b61a860037310d58f317f59573e438827d61a",
        when="@12.3.0 target=neoverse_v2",
    )
    ...
    def patch(self):
        ...
        if self.version >= Version("6"):
            filter_file(
                "@zlibdir@",
                "-L{0}".format(spec["zlib-api"].prefix.lib),
                "gcc/Makefile.in",
            )
            filter_file(
                "@zlibinc@",
                "-I{0}".format(spec["zlib-api"].prefix.include),
                "gcc/Makefile.in",
            )
    ...
    def detect_gdc(self):
        ...
        detected_packages = by_path(
            [self.name],
            path_hints=([compiler_dir] if os.path.isdir(compiler_dir) else None),
        )
        ...
        candidate_specs = [
            p.spec
            for p in filter(
                lambda p: p.spec.satisfies(required_spec),
                detected_packages.get(self.name, ()),
            )
        ]
        ...
        if not candidate_specs:
            ...
            if all(
                candidate_gdc == s.extra_attributes["compilers"]["d"]
                for s in candidate_specs[1:]
            ):
